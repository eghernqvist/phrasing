...
feature 'edit mode bubble' do
  ...
  it 'should be able to visit phrack index via edit_all icon' do
    find(:css, ".phrack-edit-all-phrases-link").click
    current_path.should == phrack_phrases_path
  end
...
feature "phrack index" do

  before do
    FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar")
    visit phrack_phrases_path
  end
  ...
  it "searches in the middles of strings" do
    FactoryGirl.create(:phrack_phrase, key: "site.index.something")
  ...
  it "can show all" do
    FactoryGirl.create(:phrack_phrase, key: "foe", value: "beer")
    click_button 'Search'
    page.should have_content 'foo'
  ...
  it 'not null values first, global order by key' do
    FactoryGirl.create(:phrack_phrase, key: "foo1", value: nil)
    FactoryGirl.create(:phrack_phrase, key: "foo2", value: "beer")
    FactoryGirl.create(:phrack_phrase, key: "foo3", value: nil)
    visit phrack_phrases_path
  ...
  context "more than one locale" do
    ...
    before do
      PhrackPhrase.destroy_all
      FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar1", locale: "en")
      FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar2", locale: "fa")
      FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar3", locale: "it")
    end

    it "nil locale, blank search" do
      # impossible for user to replicate this case
      visit phrack_phrases_path('search' => '', 'commit' => 'Search')
    ...
    it "nil locale, present search" do
      # impossible for user to replicate this case
      visit phrack_phrases_path('search' => 'foo', 'commit' => 'Search')
      page.should have_content 'bar1'
      page.should_not have_content 'bar2'
      page.should_not have_content 'bar3'
      visit phrack_phrases_path('search' => 'fuu', 'commit' => 'Search')
      page.should_not have_content 'foo'
    end

    it "blank locale, foo search" do
      # impossible for user to replicate this case
      visit phrack_phrases_path('locale' => '', 'commit' => 'Search')
      page.should have_content 'foo'
    end
    ...
    it "present locale, foo search" do
      # impossible for user to replicate this case
      visit phrack_phrases_path('locale' => 'en', 'commit' => 'Search')
      page.should have_content 'foo'
    end
...
feature "phrack edit" do
  before do
    FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar")
    visit phrack_phrases_path
  end
  after do
    PhrackPhrase.delete_all
  end
  ...
  scenario "visit edit form" do
    ...
    click_button 'Search'
    click_link 'foo'
    fill_in "phrack_phrase[value]", with: 'baz'
  end
end

feature "phrack update" do
  before do
    FactoryGirl.create(:phrack_phrase, key: "foo", value: "bar")
    visit phrack_phrases_path
    fill_in 'search', with: 'foo'
  ...
  scenario "update" do
    ...
    click_button "Update"
    current_path.should == phrack_phrases_path
    PhrackPhrase.find_by_key("foo").value.should == 'baz'
...
feature "downloading and uploading yaml files" do
  ...
  it "round-trips the YAML" do
    FactoryGirl.create(:phrack_phrase, key: "a.foo1", value: "bar1")
    FactoryGirl.create(:phrack_phrase, key: "a.foo2:", value: "bar2")
    FactoryGirl.create(:phrack_phrase, key: "a.b.foo3", value: "bar3")
    FactoryGirl.create(:phrack_phrase, key: "c.foo4", value: "bar4")
    FactoryGirl.create(:phrack_phrase, key: 2, value: "bar5")
    assert PhrackPhrase.count == 5

    visit import_export_phrack_phrases_path

    click_link 'Download as YAML'
    PhrackPhrase.destroy_all
    assert PhrackPhrase.count == 0

    yaml = page.source
    file = Tempfile.new 'phrack'
    file.write yaml
    file.close
    ...
    assert PhrackPhrase.count == 5
    assert PhrackPhrase.find_by_key("a.foo1").value == "bar1"
    assert PhrackPhrase.find_by_key("a.foo2:").value == "bar2"
    assert PhrackPhrase.find_by_key("a.b.foo3").value == "bar3"
    assert PhrackPhrase.find_by_key("c.foo4").value == "bar4"
    assert PhrackPhrase.find_by_key(2).value == "bar5"
  end

  it "round-trips the yaml with complicated text" do
    value = "“hello world“ üokåa®fgsdf;::fs;kdf"
    FactoryGirl.create(:phrack_phrase, key: "a.foo", value: value)
    ...
    click_button "Upload"
    file.unlink
    assert PhrackPhrase.find_by_key("a.foo").value == value
  end
...
feature "locales" do
  ...
  it "imports yaml containing multiple locales" do
    ...
    assert PhrackPhrase.count == 2
    a = PhrackPhrase.where(locale: 'en').first
    assert a.key == 'hello'
    assert a.value == 'world'
    b = PhrackPhrase.where(locale: 'es').first
    assert b.key == 'hello'
    assert b.value == 'mundo'
  end

  it "exports yaml containing multiple locales" do
    FactoryGirl.create(:phrack_phrase, locale: 'en', key: 'hello', value: 'world')
    FactoryGirl.create(:phrack_phrase, locale: 'es', key: 'hello', value: 'mundo')

    visit download_phrack_phrases_path
    yaml = page.source
